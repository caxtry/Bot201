  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    for (const itemarray in data.items){
      ...
      switch(itemarray.toLowerCase()){
        ...
        case "rock": theitems.push(`🪨 ${data.items[`${itemarray}`]} Rock${data.items[`${itemarray}`] > 1 ? "s":""} | \`${nFormatter(5 * data.items[`${itemarray}`])} 💸\``);
        break;
    
        // Pets cases
        case "fish": theitems.push(`🐟 ${data.items[`${itemarray}`]} Fish${data.items[`${itemarray}`] > 1 ? "s":""} | \`${nFormatter(1000*data.items[`${itemarray}`])} 💸\``); break;
        ...
        case "rui": theitems.push(`🐱 ${data.items[`${itemarray}`]} Rui${data.items[`${itemarray}`] > 1 ? "s":""} | \`${nFormatter(2000 * data.items[`${itemarray}`])} 💸\``); break;
        case "doockie": theitems.push(`🐱${data.items[`${itemarray}`]} Dog${data.items[`${itemarray}`] > 1 ? "s":""} | \`${nFormatter(2000 * data.items[`${itemarray}`])} 💸\``); break;
      }
    }
    //return some message!
    return message.reply({embeds: [new MessageEmbed()
