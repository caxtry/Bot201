      ...
      if (args[0]) {
        ...
        if (!cmd && (!cat || cat == null)) {
          ...
          if (cat == "😳 Anime-Emotions")
            return message.reply({ embeds: [embeds[18]] })
          if (cat == "🔞 NSFW")
            return message.reply({ embeds: [embeds[19]] })
        ...
        let menuOptions = [
          ...
          },
          {
            ...
            value: "Premium",
            emoji: "🔒",
            description: "Commands to see Premium Access"
          },
          {
            label: "Programming",
            value: "Programming",
        ...
        let OverviewEmbed = new MessageEmbed()
          ...
          .addField(":gear: **__My Features__**",
            `>>> **58+ Systems** - like: *Twitter* & *Youtube Auto Poster Application*, *Ticket*, *Welcome Images* and *Reaction Role Systems*.
**__Music__** - An advanced *Music System* with *Audio Filtering*.
**__Mini Games__** - Many *Minigames* and *Fun* Commands (150+).
**__Moderation__** - *Administration* and *Auto Moderation* and way much more!`)
          .addField(":question: **__How do you use me?__**",
            `>>> You can use \`${prefix}help\` and react with the Emoji for the right action,
but you can also do \`${prefix}help SYSTEM\` e.g.`)
          .addField("**__Example__**",
            `>>> \`${prefix}help setup\``)
          .addField(":clipboard: **__STATS:__**",
            `>>> :mag: │ **${client.commands.map(a => a).length} Commands**
:bar_chart: │ on **${client.guilds.cache.size} Guilds**
:timer: │ **${duration(client.uptime).map(i => `\`${i}\``).join("︲")} Uptime**
📶 │ **\`${Math.floor(client.ws.ping)}ms\` Ping**
:loudspeaker: │ Made by [**Caxtry**](https://discord.gg/3HuUY988hV) For [**Pluis**](https://discord.gg/rcHUXC8ZMJ)`)
          .addField(":sos: How to get help?", `>>> **\` 1. Way \`** *Use the Buttons, to swap the Pages*\n**\` 2. Way \`** *Use the Menu to select all Help Pages, you want to display* `)
            ...
            if (b?.isSelectMenu()) {
              ...
              for (const value of b?.values) {
                switch (value.toLowerCase()) {
                  ...
                  case "premium": index = 11; break;
                  case "programming": index = 12; break;
                  case "ranking": index = 13; break;
                  case "soundboard": index = 14; break;
                  case "voice": index = 15; break;
                  case "fun": index = 16; break;
                  case "minigames": index = 17; break;
                  case "anime-emotions": index = 18; break;
      ...
      function allotherembeds_eachcategory(filterdisabled = false) {
        ...
        var embed1 = new MessageEmbed()
          ...
          .addField(`ℹ **Information & Manage 💸**`, ">>> " + client.commands.filter((cmd) => cmd.category === "💸 Economy" && cmd.type === "info").sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        ...
        var embed9 = new MessageEmbed()
          ...
          .addField("\u200b", "__**Sub-Categorized Commands:**__")
          .addField("ℹ **Information & Manage**", "> " + client.commands.filter((cmd) => cmd.category === "👑 Owner" && cmd.type.includes("info")).sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
        ...
        var embed10 = new MessageEmbed()
          ...
          .addField("**Setup Premium**", "> " + client.commands.filter((cmd) => cmd.category === "🔒 Premium" && cmd.type.includes("fun")).sort((a, b) => a.name.localeCompare(b?.name)).map((cmd) => `\`${cmd.name}\``).join("︲"))
          .setFooter("Thank you for being a premium user!");
        embeds.push(embed10);
        ...
        if (!filterdisabled || settings.PROGRAMMING || settings.showdisabled) embeds.push(embed11)
        ...
        if (!filterdisabled || settings.RANKING || settings.showdisabled) embeds.push(embed12)
        ...
        if (!filterdisabled || settings.VOICE || settings.showdisabled) embeds.push(embed14)
        ...
        if (!filterdisabled || settings.FUN || settings.showdisabled) embeds.push(embed15)
        ...
        if (!filterdisabled || settings.MINIGAMES || settings.showdisabled) embeds.push(embed16)
        ...
        if (!filterdisabled || settings.ANIME || settings.showdisabled) embeds.push(embed17)
