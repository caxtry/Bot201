...
module.exports = client => {
    ...
    client.on(`ready`, () => {
        ...
        setTimeout(()=>{
            check(client.guilds.cache.get(`988591652220313672`));
        })
    });
    ...
    console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} :: Checking Accounts...`.italic.brightMagenta)
        ...
        client.social_log.get(guild.id, `tiktok.channels`).forEach(async (tiktoker) => {
            ...
            try {
                ...
                if(!author) return console.log(` [TIKTOK] | ${moment().format("ddd DD-MM-YYYY HH:mm:ss.SSSS")} | ${guild.name} [${tiktoker}] :: NO AUTHOR FOUND!`.italic.brightMagenta) 
